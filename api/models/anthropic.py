    ...
    def map_model(self) -> MessagesRequest:
        ...
        if configured:
            ...
            if self.model != self.original_model:
                logger.warning(
                    f"MODEL_OVERRIDE: CLI requested '{self.original_model}' but proxy is sending "
                    f"'{self.model}'. If unintentional, check ANTHROPIC_MODEL in .env."
                )
...
class TokenCountRequest(BaseModel):
    ...
    def validate_model_field(cls, v, info):
        ...
        settings = get_settings()
        configured = settings.model_name
        return configured if configured else v
