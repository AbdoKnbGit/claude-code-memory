...
def try_optimizations(
    ...
    for handler in OPTIMIZATION_HANDLERS:
        ...
        if result is not None:
            logger.debug(f"OPTIMIZATION_HIT: {handler.__name__}")
            return result
    ...
    if _max_tok <= 10:
        ...
        for m in reversed(request_data.messages):
            if m.role == "user":
                _last_text = extract_text_from_content(m.content)
                break
        logger.debug(
            f"OPTIMIZATION_MISS: max_tokens={_max_tok} msgs={len(request_data.messages)} "
            f"has_tools={bool(request_data.tools)} has_system={bool(request_data.system)} "
            f"content={repr(_last_text[:80])}"
        )
    return None
