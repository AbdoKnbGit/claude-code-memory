...
class GlobalRateLimiter:
    ...
    def __init__(
        self,
        rate_limit: int | None = 40,
        rate_window: float | None = 60.0,
        max_concurrency: int | None = 5,
    ):
        if hasattr(self, "_initialized"):
            return

        self._rate_limit = rate_limit if rate_limit is not None else 40
        self._rate_window = float(rate_window) if rate_window is not None else 60.0
        concurrency = max_concurrency if max_concurrency is not None else 5
        ...
        self._concurrency_sem = asyncio.Semaphore(concurrency)
